...
export default function StaffDashboard() {
  ...
  const handleUpdate = async (id: number) => {
    const newTitle = prompt(
      'Enter new title',
      events.find(ev => ev.id === id)?.title
    );
    if (!newTitle) return;
    const token = localStorage.getItem('admin_token')!;
    try {
      await updateEvent(token, id, { title: newTitle });
      loadEvents();
    } catch (err) {
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <ul className="space-y-4">
        {events.map(ev => (
          <li key={ev.id} className="flex justify-between items-center border p-4 bg-white rounded">
            <div>
              <h3 className="font-bold text-lg">{ev.title}</h3>
              <p>{new Date(ev.start_date).toLocaleString()}</p>
              {ev.venue && <p>Venue: {ev.venue}</p>}
              {ev.description && <p>Description: {ev.description}</p>}
            </div>
            <div className="flex space-x-2">
              <button
